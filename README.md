...
**Η MySQL πρέπει να τρέχει πριν από αυτό.** Η εφαρμογή ανοίγει σύνδεση κατά την
εκκίνηση και, αν δεν τη βρει, τερματίζει αντί να σηκωθεί μισοάχρηστη. Το
αποτέλεσμα είναι εκατοντάδες γραμμές στοίβας που καταλήγουν σε
`Unable to determine Dialect without JDBC metadata` — παραπλανητικό μήνυμα: η
πραγματική αιτία βρίσκεται στο **τελευταίο** `Caused by`, που είναι
`java.net.ConnectException: Connection refused`.

Η προτεινόμενη «λύση» του μηνύματος, δηλαδή να οριστεί ρητά το
`hibernate.dialect`, δεν διορθώνει τίποτα: κάνει την εφαρμογή να ξεκινάει
κανονικά και να αποτυγχάνει σε κάθε αίτημα. Ξεκίνησε τη βάση.
...
**Δύο ευρετήρια δηλωμένα, τα υπόλοιπα δωρεάν.** Το σχήμα παίρνει ευρετήρια από
τρεις πηγές χωρίς να ζητηθούν: τα primary keys, κάθε στήλη `unique` (uuid,
kaek, username, vat, registry_number, τα ονόματα των καταλόγων) και -- αυτό
ξεχνιέται -- **κάθε foreign key**, γιατί η InnoDB δημιουργεί μόνη της ευρετήριο
στη στήλη που αναφέρεται. Έτσι όλες οι μονοστήλες διαδρομές καλύπτονται ήδη.

Δηλώνονται ρητά μόνο δύο σύνθετα, στα entities και όχι με το χέρι στη βάση,
ώστε να αναπαράγονται όπου κι αν στηθεί το σχήμα:

| Ευρετήριο | Πίνακας | Γιατί |
|---|---|---|
| `idx_activity_crop_date` | `field_activities (crop_id, activity_date)` | Το ημερολόγιο φιλτράρει ανά καλλιέργεια, περιορίζει εύρος ημερομηνιών και ταξινομεί κατά ημερομηνία· ο έλεγχος του χρόνου αναμονής διαβάζει την ίδια διαδρομή. |
| `idx_parcel_farmer_active` | `parcels (farmer_id, is_active)` | Η λίστα αγροτεμαχίων φιλτράρει πάντα ανά αγρότη και συχνά ανά κατάσταση. |

**Τα φίλτρα αναζήτησης δεν ευρετηριάζονται και δεν γίνεται.** Η ονομασία
αγροτεμαχίου, ο ΚΑΕΚ και το επώνυμο αναζητούνται με `LIKE '%κείμενο%'`, και το
αρχικό wildcard απαγορεύει τη χρήση ευρετηρίου. Θα απαιτούσε full-text index ή
αναζήτηση προθέματος, που θα περιόριζε τη λειτουργικότητα. Με το μέγεθος
δεδομένων της εφαρμογής το κόστος είναι αμελητέο, και η προσθήκη ευρετηρίων που
δεν χρησιμοποιούνται θα επιβάρυνε μόνο τις εγγραφές.

**Το API μιλάει ελληνικά, οι κωδικοί αγγλικά.** Κάθε κείμενο που προορίζεται
για άνθρωπο -- τα μηνύματα επικύρωσης των DTO και το `description` κάθε
`ResponseMessageDTO` -- είναι στα ελληνικά, γιατί το front-end τα εμφανίζει
αυτούσια: τα μηνύματα επικύρωσης γίνονται `mat-error` κάτω από το πεδίο και οι
κανόνες του ημερολογίου εμφανίζονται ως έχουν στη φόρμα εργασίας. Το πεδίο
`code` μένει αγγλικό (`ParcelNotFound`, `ActivityInvalidArgument`), γιατί δεν
το διαβάζει άνθρωπος: είναι το αναγνωριστικό πάνω στο οποίο διακλαδίζεται ο
πελάτης και πάνω στο οποίο γράφονται τα tests. Τα logs παραμένουν αγγλικά.
